...
from dataset.balanced_dataset import ViVit_KFold
...
def run_embed2level_1fold(feature_dir, gt_csv, nb_fold, dico_level, layers_config, exp_name,version_name,saving_dir):
    ...
    train_dataset = ViVit_KFold(feature_dir, gt_csv, classification="level",split="train", nb_fold = nb_fold, dico_level =  dico_level)
    val_dataset = ViVit_KFold(feature_dir, gt_csv, classification="level",split="val", nb_fold = nb_fold, dico_level =  dico_level)
    test_dataset = ViVit_KFold(feature_dir, gt_csv, classification="level",split="test", nb_fold = nb_fold, dico_level = dico_level )
...
def process_Vivit(feature_dir = "/media/LaCie/Features/ViVit/ViVit" , annotation_dir = "/media/LaCie/balanced_level",saving_dir = "/home/julie/Results/LinearProbing/Balanced_ViVit"):
    """Run the classification for several configuration with features extracted from ViVit
    ...
    file_names = {"HN_S.csv" : {"Hard Neg":0, "Sure":1},
                "EN_S.csv":{"Easy Neg":0, "Sure":1}}

 
    for file_name,dico_level in file_names.items():
        output_size = len(dico_level)
        model_configs = {"small": [768,128,output_size]}

        for k_model, layers_config in model_configs.items():
            
            gt_file = os.path.join(annotation_dir,  "02_"+file_name)
            exp_name = "ViVit_" + file_name.split(".")[0]
