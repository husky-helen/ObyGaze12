from dataset.balanced_dataset import XCLIPLSMDC_KFold
...
def run_embed2level_1fold(feature_dir, gt_csv, nb_fold, dico_level, layers_config, exp_name,version_name,saving_dir):
    ...
    train_dataset = XCLIPLSMDC_KFold(feature_dir, gt_csv, classification="level",split="train", nb_fold = nb_fold, dico_level =  dico_level)
    val_dataset = XCLIPLSMDC_KFold(feature_dir, gt_csv, classification="level",split="val", nb_fold = nb_fold, dico_level =  dico_level)
    test_dataset = XCLIPLSMDC_KFold(feature_dir, gt_csv, classification="level",split="test", nb_fold = nb_fold, dico_level = dico_level )
...
def process_XCLIP_LSMDC(feature_dir , annotation_dir, saving_dir):
    ...
    for file_name,dico_level in file_names.items():
        ...
        for k_model, layers_config in model_configs.items():
            ...
            exp_name = "XCLIPLSMDC_" +  file_name.split(".")[0]
